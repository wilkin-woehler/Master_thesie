...
\newText{
Understanding and describing phase transitions has always been as important as it has been challenging. On the one side its importance can be see in technological disciplines like metllurgy or in present scientific fields like atmosphere physics, where the properties of phase transitions are decisive. On the other side the challenge is due to the large number of participating particles.\\\\
Nevertheless early on a concept to qualitatively describe phase transitions was developed, called clasical nucleation theory fro todays point of view. It assumes a free energy landscape for a collective observable describing the transition, that includes a barrier seperating the two distinct phases. Due to thermic Markovian fluctuations the system will eventually cross the barrier and the averages for this process is given by the Temperature and the barrier height.\\\\
It is discussed in \autoref{sec:CNT} and further used a few times during this thesis. However it has been shown, for example by Sear 2012\cite{Sear2012}, that in many circumstances CNT fails to provide a quantiative description. As this is not only unsatisfying for a theorist, but actually of importance in many fields of science, various approaches to solve this problem have been proposed in the past. A comprehensive list of them can be found by Kuhnbold et al.\cite{Kuhnbold2019}. It includes modifying the free energy landscape, choosing suitable reaction coordinates or to include non-Markovian effects in the description. In the aforementioned article by Kuhnbold, the presences of these is shown for the largest cluster in the metastable Lennard-Jones system. Similarly Pelagejcev 2019\cite{ThesisPhilipp} lately found non-neglibile memory kernels in the same system, by employing a numeric memory kernel reconsturction scheme derived by Meyer et al. 2019\cite{Meyer2019a}. In this thesis we continue to extend the examples of memory kernel studies by adding our findings in the elastic hard sphere system in \autoref{sec:memory_kernels}.\\\\
While the theory and framework to obtain the evaluated non-staitionary memory kernel it too broad to cover at this point, we may mention the foundations as given by Meyer 2020\cite{MeyerThesis} in his dissertation.\\\\
...
\newText{To obtain information about the nucleation state of the experiment also diferent approaches are used. Because the information of each single particle position is most of the time not accessible in experiments, light scattering is pften used to determine the crystallinity of a sample and the nucleation time then can be defined by when an overall crystallinity is reached. In comparisontheorists have access to all positions but no simple measurement as light scattering available. Instead they use a variety of cluster finding algorithms to determine crystalline local structures. The nucleation time then might be defined by the mean waiting time until a stable crystalline phase is found, for example Filion et al. 2010\cite{Filion2010a}. A discussion on possible definitions of the nucleation time is done in \autoref{sec:nucleation_times}. Furthermore wihtin this thesis we discuss the nucleation behavior of the common simulation geometry in \autoref{sec:induction_time_expectation} and the corresponding one for the infinitely large volume in \autoref{sec:inf_vol_limit_nuc_rate}, leading to a possible explanation of the discrepancy between the nucleation rates measured in simulations and laboratory based experiments.
}
%While the expected behavior of disjunct volumes under the assumption of a constant nucleation rate is discussed in \autoref{sec:induction_time_expectation}, the overall solid fraction in the thermodynamic limit of infinite volume is inspected in \autoref{sec:nucleation_rates}.\\
%At last we may also note the difference in defining nucleation rates, as it 
%\todo{Some of this introduction stuff may better fit into comparison to real world?}\\
%Nucleation rates for the metastable hard sphere fluid have been measured on the experimental as well as on the theoretical side, but with a large discrepancy as discussed in \autoref{sec:comparison}. The employed procedures and definitions also vary but not to a point to explain the discrepancy so far. The differences mostly originate from the kind of accessible system and information. While the experimentalists often have access to very large systems but without knowing all positions at all times, theorists mostly have smaller systems in numerical simulations but with the advantage of being able to access all particle positions and in case of MD simulations their velocities as well.\\
